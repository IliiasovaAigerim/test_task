...
RSpec.describe MultipleAuthorsIpsSerializer do
  describe '#as_json' do
    let(:serializer_output) { described_class.new(input_data).as_json }
    ...
    context 'when input is present' do
      ...
      it 'returns an array of hashes' do
        expect(serializer_output).to be_an(Array)
        expect(serializer_output.first).to be_a(Hash)
      end

      it 'correctly maps IP addresses and logins' do
        expect(serializer_output).to contain_exactly(
    ...
    context 'when input is empty' do
      ...
      it 'returns an empty array' do
        expect(serializer_output).to eq([])
      end
    end
  end
end
